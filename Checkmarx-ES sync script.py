...
class CheckmarxClient:
    ...
    def get_all_users(self) -> List[Dict]:
        """Get all users from Checkmarx."""
        url = f"{self.base_url}/cxrestapi/auth/users"
        
        try:
            response = self.session.get(url)
            response.raise_for_status()
            users = response.json()
            logger.info(f"Retrieved {len(users)} users from Checkmarx")
            return users
        except requests.exceptions.RequestException as e:
    ...
    def get_user_teams(self, user_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.debug(f"Failed to get teams for user {user_id}: {e}")
            return []
    ...
    def get_team_memberships(self, use_alternative_method: bool = False, use_projects_method: bool = False) -> Dict[str, Set[str]]:
        ...
        if use_projects_method:
            ...
            # Original method: Get teams, then query users for each team
            skipped_teams = []
            teams = self.get_teams()
